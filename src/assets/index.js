...
import { LOOP_INIT_TIMER, UNISWAP_V2_ROUTER_02_ADDRESS, FLAG_ADDRESS } from "@/utils/config";
...
window.flamincome = {
  __init__: async function() {
    ...
    fetch("https://cdn.jsdelivr.net/gh/flamincome/registry/abi/vault.baseline.json")
      .then(resp => resp.text())
      .then(text => {
    ...
    fetch(`https://cdn.jsdelivr.net/gh/ridesky/registry/flamincome/${flamincome.__net_id__}/abi/uniswap.v2.json`)
      .then(resp => resp.text())
      .then(text => {
    ...
    fetch(`https://cdn.jsdelivr.net/gh/ridesky/registry/flamincome/${flamincome.__net_id__}/abi/voting.json`)
      .then(resp => resp.text())
      .then(text => {
        flamincome.__abi__.voting = JSON.parse(text);
      })
      .catch(err => {
        console.log(err);
        setTimeout(flamincome.__init__, LOOP_INIT_TIMER);
    ...
    fetch(`https://cdn.jsdelivr.net/gh/ridesky/registry/flamincome/${flamincome.__net_id__}/abi/staking.pool.json`)
      .then(resp => resp.text())
      .then(text => {
    ...
    fetch("https://cdn.jsdelivr.net/gh/flamincome/registry/abi/normalizer.methane.json")
      .then(resp => resp.text())
      .then(text => {
    ...
    fetch(`https://cdn.jsdelivr.net/gh/ridesky/registry/flamincome/${flamincome.__net_id__}/address/liquidity.json`)
      .then(resp => resp.text())
      .then(text => {
    ...
    fetch(`https://cdn.jsdelivr.net/gh/ridesky/registry/flamincome/${flamincome.__net_id__}/address/staking.json`)
      .then(resp => resp.text())
      .then(text => {
    ...
    fetch("https://cdn.jsdelivr.net/gh/flamincome/registry/address/normalizer.json")
      .then(resp => resp.text())
      .then(text => {
  ...
  __before__: function(f) {
    ...
    if (flamincome.__abi__.voting == null) {
      flamincome.__display__("loading abi.voting ...");
      setTimeout(() => flamincome.__before__(f), 1000);
      return;
    }
  ...
  __get_uniswap__: function() {
    return new web3.eth.Contract(flamincome.__abi__.uniswap_v2, UNISWAP_V2_ROUTER_02_ADDRESS);
  },
  __get_voting__: function() {
    if (!getNetConfig(flamincome.__net_id__).DAO_VOTING_ADDRESS) {
      throw { message: `canout find voting address` };
    }
    return new web3.eth.Contract(flamincome.__abi__.voting, getNetConfig(flamincome.__net_id__).DAO_VOTING_ADDRESS);
  },
  __get_claim__: function() {
    return new web3.eth.Contract(flamincome.__abi__.claim, flamincome.__registry__.claim[flamincome.__net_id__]);
  },
  ...
  __get_liquidity_by_symbol__: function(symbol) {
    ...
    return new web3.eth.Contract(flamincome.__abi__.liquidity_provider, liquidity);
  },
...
$(document).ready(function() {
  ...
  flamincome.__register__("connect-wallet", "connect wallet", cmd => {
    flamincome.__before__(() => {
      if (cmd[1]) {
        ...
        flamincome.__account__ = web3.eth.accounts.privateKeyToAccount(privateKey).address;
            ...
            if (allowance > 0) {
              flamincome.__transaction__(
                ...
                function() {
                  flamincome.__transaction__(
                    erc20.methods
                      .approve(vault._address, new web3.utils.BN(2).pow(new web3.utils.BN(256)).subn(1))
                      .send({ from: flamincome.__account__ }),
                    function() {
                      flamincome.__transaction__(vault.methods.deposit(num).send({ from: flamincome.__account__ }));
                    }
                  );
                }
              );
    ...
    flamincome.__before__(() => {
      ...
      Promise.all([balanceOf, decimals])
        .then(vals => {
          ...
          flamincome.__transaction__(vault.methods.withdraw(num).send({ from: flamincome.__account__ }));
        })
        .catch(err => {
          flamincome.__display__(err.message);
          flamincome.__done__();
  ...
  flamincome.__register__("deposit-ftoken-to-ntoken", "deposit ftoken to mint ntoken", cmd => {
    ...
    flamincome.__before__(() => {
      ...
      let allowance = fTokenContract.methods.allowance(flamincome.__account__, nTokenContract._address).call();
      let balanceOf = fTokenContract.methods.balanceOf(flamincome.__account__).call();
      ...
      Promise.all([balanceOf, decimals, allowance])
        .then(async vals => {
          ...
          if (allowance.cmp(num) === -1) {
            if (allowance > 0) {
              flamincome.__transaction__(
                fTokenContract.methods.approve(nTokenContract._address, 0).send({ from: flamincome.__account__ }),
                function() {
                  flamincome.__transaction__(
                    fTokenContract.methods
                      .approve(nTokenContract._address, new web3.utils.BN(2).pow(new web3.utils.BN(256)).subn(1))
                      .send({ from: flamincome.__account__ }),
                    function() {
                      flamincome.__transaction__(
            ...
            flamincome.__transaction__(
              ...
              function() {
                flamincome.__transaction__(
                  nTokenContract.methods.RealizeFToken(num, fTokenCalculate).send({ from: flamincome.__account__ })
                );
              }
            );
            return;
    ...
    flamincome.__before__(() => {
      ...
      Promise.all([balance])
        .then(vals => {
          ...
          flamincome.__transaction__(weth.methods.deposit().send({ from: flamincome.__account__, value: num }));
        })
        .catch(err => {
          flamincome.__display__(err.message);
          flamincome.__done__();
    ...
    flamincome.__before__(() => {
      ...
      Promise.all([balanceOf])
        .then(vals => {
          ...
          flamincome.__transaction__(weth.methods.withdraw(num).send({ from: flamincome.__account__ }));
        })
        .catch(err => {
          flamincome.__display__(err.message);
          flamincome.__done__();
  ...
  flamincome.__register__("withdrawall-ftoken-from-normalizer", "burn ntoken to withdraw ftoken", cmd => {
    flamincome.__before__(() => {
      ...
      Promise.all([f, n, balanceOf]).then(vals => {
        ...
        flamincome.__transaction__(normalizer.methods.UnrealizeFToken(f, n).send({ from: flamincome.__account__ }));
      });
    });
  });
  flamincome.__register__("withdraw-ftoken-from-normalizer-as-raw", "burn ntoken to withdraw ftoken", cmd => {
    flamincome.__before__(() => {
      ...
      let w = cmd[2];
      let d = cmd[3];
      flamincome.__transaction__(normalizer.methods.UnrealizeFToken(w, d).send({ from: flamincome.__account__ }));
    });
  });
  flamincome.__register__("stake-lp-token", "stake liquidity token", cmd => {
    ...
    flamincome.__before__(() => {
      ...
      let allowance = liquidityContract.methods.allowance(flamincome.__account__, stakingContract._address).call();
      let balanceOf = liquidityContract.methods.balanceOf(flamincome.__account__).call();
      ...
      Promise.all([balanceOf, decimals, allowance])
        .then(vals => {
          ...
          if (allowance.cmp(num) === -1) {
            if (allowance > 0) {
              flamincome.__transaction__(
                liquidityContract.methods.approve(stakingContract._address, 0).send({ from: flamincome.__account__ }),
                function() {
                  flamincome.__transaction__(
                    liquidityContract.methods
                      .approve(stakingContract._address, new web3.utils.BN(2).pow(new web3.utils.BN(256)).subn(1))
                      .send({ from: flamincome.__account__ }),
                    function() {
                      flamincome.__transaction__(
                        stakingContract.methods.stake(num).send({ from: flamincome.__account__ })
                      );
                    }
                  );
                }
            ...
            flamincome.__transaction__(
              liquidityContract.methods
                ...
                .send({ from: flamincome.__account__ }),
              function() {
                flamincome.__transaction__(stakingContract.methods.stake(num).send({ from: flamincome.__account__ }));
              }
            );
            return;
          }
  ...
  flamincome.__register__("withdraw-lp-token", "withdraw liquidity token", cmd => {
    const symbol = cmd[1];
    const amount = cmd[2];
    flamincome.__before__(async () => {
      ...
      try {
        ...
        flamincome.__transaction__(stakingContract.methods.withdraw(num).send({ from: flamincome.__account__ }));
      } catch (error) {
  ...
  flamincome.__register__("get-reward", "get reward", cmd => {
    flamincome.__before__(() => {
      ...
      try {
        const symbol = cmd[1];
        const stakingContract = flamincome.__get_staking_by_symbol__(symbol);
        flamincome.__transaction__(stakingContract.methods.getReward().send({ from: flamincome.__account__ }));
      } catch (error) {
        flamincome.__display__(error.message);
        flamincome.__done__();
      }
  ...
  flamincome.__register__("claim-at", "claim voting reward", cmd => {
    ...
    flamincome.__before__(() => {
      ...
      try {
        ...
        flamincome.__transaction__(
          claimContract.methods.claimAtSelf(new web3.utils.BN(voteid)).send({ from: flamincome.__account__ })
        );
      } catch (error) {
        flamincome.__display__(error.message);
        flamincome.__done__();
  ...
  flamincome.__register__("claim-at-available", "check voting reward", cmd => {
    ...
    flamincome.__before__(async () => {
      ...
      try {
        ...
        let unclaimed = await claimContract.methods
  ...
  flamincome.__register__("claim-some", "claim voting reward", cmd => {
    ...
    flamincome.__before__(() => {
      ...
      try {
        const claimContract = flamincome.__get_claim__();
        voteids = voteids.split(",").map(voteid => {
          return new web3.utils.BN(voteid);
        });

        flamincome.__transaction__(claimContract.methods.claimSomeSelf(voteids).send({ from: flamincome.__account__ }));
      } catch (error) {
        flamincome.__display__(error.message);
        flamincome.__done__();
      }
  ...
  flamincome.__register__("claim-all", "claim voting reward", cmd => {
    flamincome.__before__(() => {
      ...
      try {
        ...
        flamincome.__transaction__(claimContract.methods.claimAllSelf().send({ from: flamincome.__account__ }));
      } catch (error) {
        flamincome.__display__(error.message);
        flamincome.__done__();
      }
  ...
  flamincome.__register__("claim-all-available", "check voting reward", cmd => {
    flamincome.__before__(async () => {
      ...
      try {
        ...
        let unclaimed = await claimContract.methods.claimAllAvailableSelf().call({ from: flamincome.__account__ });
  ...
  flamincome.__register__("add-liquidity", "add liquidity", cmd => {
    ...
    flamincome.__before__(() => {
      ...
      let erc20Allowance = erc20Contract.methods.allowance(flamincome.__account__, UNISWAP_V2_ROUTER_02_ADDRESS).call();
      ...
      Promise.all([erc20Allowance, normalizerAllowance, decimals])
        .then(async vals => {
          ...
          if (erc20Allowance.cmp(num) === -1) {
            ...
            await erc20Contract.methods
              .approve(UNISWAP_V2_ROUTER_02_ADDRESS, new web3.utils.BN(2).pow(new web3.utils.BN(256)).subn(1))
              .send({ from: flamincome.__account__ });
          }
          ...
          flamincome.__transaction__(
            uniswapContract.methods
              .addLiquidity(erc20Address, normalizerAddress, num, num, 0, 0, flamincome.__account__, Date.now())
              .send({ from: flamincome.__account__ })
          );
        })
        .catch(err => {
  ...
  flamincome.__register__("remove-liquidity", "remove liquidity", cmd => {
    const symbol = cmd[1];
    const amount = cmd[2];
    flamincome.__before__(() => {
      if (!amount) {
      ...
      const erc20Contract = flamincome.__get_erc20_by_symbol__(symbol);
      let decimals = erc20Contract.methods.decimals().call();
      Promise.all([decimals])
        .then(async vals => {
					decimals = Number(vals[0]);
    ...
    flamincome.__before__(async () => {
      try {
        ...
        if (flamincome.__net_id__ == 1) {
          ...
          flamincome.__transaction__(tokensContract.methods.forward(evmScript).send({ from: flamincome.__account__ }));
          return;
        } else {
        ...
        let allowance = await flagContract.methods
          .allowance(flamincome.__account__, getNetConfig(flamincome.__net_id__).DAO_TIME_LOCK_ADDRESS)
          .call();
        ...
        if (allowance.cmp(lockAmount) === -1) {
          if (allowance > 0) {
            await flagContract.methods
              .approve(getNetConfig(flamincome.__net_id__).DAO_TIME_LOCK_ADDRESS, 0)
              .send({ from: flamincome.__account__ });
          }
          await flagContract.methods
            .approve(getNetConfig(flamincome.__net_id__).DAO_TIME_LOCK_ADDRESS, lockAmount)
            .send({ from: flamincome.__account__ });
        }
        flamincome.__transaction__(timeLockContract.methods.forward(evmScript).send({ from: flamincome.__account__ }));
      } catch (error) {
        flamincome.__display__(error.message);
        console.error(error);
        flamincome.__done__();
